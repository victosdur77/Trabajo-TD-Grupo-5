...
tabladominadores= function(X, favorable=TRUE){
  if(favorable){
    ...
    for (i in 1:(nrow(X)-1)) {
      for (j in (i+1):nrow(X)){
        if (Altmaxe[j]-Altmaxe[i]+Altmine[i]-Altmine[j]!=0){optimo=c(optimo,solve(Altmaxe[j]-Altmaxe[i]+Altmine[i]-Altmine[j],
                                                                                  Altmine[i]-Altmine[j]) )
    ...
    #Nos quitamos los cortes no importantes
    if(i>1){
...
tabladominadores(tabla1, favorable=FALSE)
